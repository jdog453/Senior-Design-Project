...
Set up the device.    1
Turn on the raspberry pi device.    2
Train the facial recognition model to recognize specific faces.    2
Register/Login to the webapp.    3
FAQs    4
Why is the nerf gun turret not turning on?    4
Why is the nerf gun turret on but not firing?    4
...
Note: We’ve also included a script that will take a video and read its frames. These frames can be used as images for the directory. See Collecting Pictures section.
